...
from sklearn.metrics import classification_report, precision_recall_curve, roc_auc_score, precision_recall_curve, auc
...
BASE_OUTPUT = "C:/Users/Diana/Desktop/data_balance/SMOTE"


# Base 2
#CSV_PATH = "C:/Users/Diana/Desktop/ieee-fraud-detection/ieee_ready_depurada.csv"
#BASE_OUTPUT = "C:/Users/Diana/Desktop/data_balance_IEEE/SMOTE_SVM"

# Base 3 = base 2 balanceada
#CSV_PATH = "C:/Users/Diana/Desktop/ieee-fraud-detection/ieee_balanced.csv"
#BASE_OUTPUT = "C:/Users/Diana/Desktop/data_balance_IEEE_balanced/SMOTE"
...
def apply_smote_on_train_data(X_train_scaled: pd.DataFrame, y_train: pd.Series, random_seed: int) -> (pd.DataFrame, pd.Series):
    ...
    k_neighbors = min(3, minority_class_count - 1) # se ajusto de 5 a 3 
    if k_neighbors < 1:
    ...
    logging.info(f"Tiempo de sobremuestreo (SMOTE): {resampling_time:.4f} segundos")
...
#    X_train_final, y_train_final, resampling_time = apply_smote_on_train_data(X_train_scaled, y_train, random_seed)
...
def evaluate_svm_model(model, X_test, y_test):
    ...
    # --- NUEVAS MÉTRICAS: ROC-AUC y PR-AUC ---
    roc_auc = roc_auc_score(y_test, y_probs)
    pr_auc = auc(recalls, precisions) # Calcula el área usando los puntos de Precision-Recall
...
# SMOTE se aplica exclusivamente sobre ese training fold, y los modelos
# se evalúan sobre el validation fold original.
#
# Las métricas se promedian sobre los 5 folds. 
# =========================================================

def run_cv(df, target_col, model_runners, random_seed, time_records):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        # CAMBIO CLAVE: SMOTE SOLO EN TRAIN
        X_res, y_res, r_time = apply_smote_on_train_data(
        ...
        for model_name, (train_func, eval_func) in model_runners.items():
            ...
            model, train_time = train_func(
                ...
                BASE_OUTPUT, f"cv_{model_name}_fold{fold}", random_seed
            )


            # Guardar tiempo crudo por fold y modelo.
            # Esta es la unidad base para el resumen final de tiempos:
            # tiempo total por fold = tiempo de SMOTE + tiempo de entrenamiento del modelo.
            time_records.append({
                'tecnica': 'SMOTE',
...
if __name__ == "__main__":
    ...
    for i in range(1, N_SIMULATIONS + 1):
        ...
        cv_results = run_cv(df_original, TARGET_COLUMN_NAME, model_runners, current_seed, time_records)
        
        for model_name in model_runners.keys():
            logging.info(f"\n--- Procesando Modelo: {model_name} con datos SMOTE ---")
            # Metricas de clasificacion para SMOTE
            sim_results[f'recall_{model_name}_SMOTE'] = cv_results.get(f'recall_{model_name}')
            sim_results[f'roc_auc_{model_name}_SMOTE'] = cv_results.get(f'roc_auc_{model_name}')
            sim_results[f'pr_auc_{model_name}_SMOTE'] = cv_results.get(f'pr_auc_{model_name}')
            sim_results[f'precision_{model_name}_SMOTE'] = cv_results.get(f'precision_{model_name}')
            sim_results[f'f1_{model_name}_SMOTE'] = cv_results.get(f'f1_{model_name}')
            sim_results[f'accuracy_{model_name}_SMOTE'] = cv_results.get(f'accuracy_{model_name}')
    ...
    if all_results:
        ...
        stats_summary = "Estadísticas de Robustez (SMOTE + 5-Fold CV)\n"
        stats_summary += "="*60 + "\n"
        ...
        # TIEMPOS
        stats_summary += "\n### TIEMPOS ###\n"
        stats_summary += "Tiempo considerado por fold: tiempo de SMOTE + tiempo de entrenamiento del modelo.\n"
        ...
        if time_records:
            ...
            for _, row in resumen_tiempos.iterrows():
                stats_summary += f"--- total_time_{row['model']}_SMOTE ---\n"
        ...
        summary_csv_path = os.path.join(BASE_OUTPUT, 'ALL_MODELS_SMOTE_summary.csv')
        results_df.to_csv(summary_csv_path, index=False)
        print(f"Resultados guardados en: {summary_csv_path}")

        # Guardar resumen estadístico
        stats_summary_path = os.path.join(BASE_OUTPUT, 'Statistics_summary_SMOTE.txt')
