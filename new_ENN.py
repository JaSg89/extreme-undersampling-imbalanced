...
# Import para Edited Nearest Neighbours (ENN)
...
#BASE_OUTPUT = "C:/Users/Diana/Desktop/data_balance/Recall_scenario2/ENN"
BASE_OUTPUT = "C:/Users/Diana/Desktop/CODIGOS_FINALES/ECCD/ENN"


# Base 2
#CSV_PATH = "C:/Users/Diana/Desktop/ieee-fraud-detection/ieee_ready_depurada.csv"
#BASE_OUTPUT = "C:/Users/Diana/Desktop/data_balance_IEEE/ENN"

# Base 3 = base 2 balanceada
#CSV_PATH = "C:/Users/Diana/Desktop/ieee-fraud-detection/ieee_balanced.csv"
#BASE_OUTPUT = "C:/Users/Diana/Desktop/data_balance_IEEE_balanced/ENN"
...
# =========================================================
# <<< Implementación de Edited Nearest Neighbours (ENN) >>>
# =========================================================

def apply_enn_on_train_data(X_train_scaled: pd.DataFrame, y_train: pd.Series, random_seed: int):
    """
    Aplica Edited Nearest Neighbours (ENN) sobre el conjunto de entrenamiento.
    Elimina muestras cuya clase no coincide con la mayoría de sus vecinos.
    """
    logging.info("Aplicando Edited Nearest Neighbours (ENN) sobre el conjunto de entrenamiento...")

    start_time = time.perf_counter()

    enn = EditedNearestNeighbours(
        n_neighbors=3,   # default típico
        kind_sel='all'   # criterio más estricto
    )

    X_res, y_res = enn.fit_resample(X_train_scaled, y_train)

    resampling_time = time.perf_counter() - start_time
    logging.info(f"Tiempo de ENN: {resampling_time:.4f} segundos")

    X_res_df = pd.DataFrame(X_res, columns=X_train_scaled.columns)
    y_res_series = pd.Series(y_res, name=y_train.name)

    logging.info(f"ENN: Tamaño del conjunto de entrenamiento original: {len(X_train_scaled)}, nuevo tamaño: {len(X_res_df)}")
    logging.info(f"Distribución de clases DESPUÉS de ENN: {dict(y_res_series.value_counts())}")

    return X_res_df, y_res_series, resampling_time
...
#    X_train_final, y_train_final, resampling_time = apply_rus_on_train_data(X_train_scaled, y_train, random_seed)
#
...
def train_xgb_model(X_train, y_train, output_dir, exp_name, random_seed):
    ...
    model = xgb.XGBClassifier(**current_xgb_params, early_stopping_rounds=25)
    start_time = time.perf_counter()
    model.fit(
...
def run_cv(df, target_col, model_runners, random_seed, output_dir, time_records):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        # ENN SOLO EN TRAIN
        X_res, y_res, r_time = apply_enn_on_train_data(
        ...
        for model_name, (train_func, eval_func) in model_runners.items():
            ...
            # tiempo total por fold = tiempo de ENN + tiempo de entrenamiento del modelo.
            time_records.append({
                'tecnica': 'ENN',
...
if __name__ == "__main__":
    ...
    for i in range(1, N_SIMULATIONS + 1):
        # *** CAMBIO IMPORTANTE: Nomenclatura para ENN ***

        sim_start_time = time.perf_counter()

        exp_name_sim = f"ENN_Compare_Sim_{i}"
        ...
        print(f"\n{'='*25} INICIANDO SIMULACIÓN {i}/{N_SIMULATIONS} (TÉCNICA: ENN) {'='*25}")
        logging.info(f"=================================================")
        logging.info(f"=== Iniciando Simulación Comparativa con ENN: {i}/{N_SIMULATIONS} ===")
        ...
        for model_name in model_runners.keys():
            logging.info(f"\n--- Procesando Modelo: {model_name} con datos de ENN ---")
            # ***  Nomenclatura de columnas para ENN ***
            sim_results[f'recall_{model_name}_ENN'] = cv_results.get(f'recall_{model_name}')
            sim_results[f'roc_auc_{model_name}_ENN'] = cv_results.get(f'roc_auc_{model_name}')
            sim_results[f'pr_auc_{model_name}_ENN'] = cv_results.get(f'pr_auc_{model_name}')
            sim_results[f'precision_{model_name}_ENN'] = cv_results.get(f'precision_{model_name}')
            sim_results[f'f1_{model_name}_ENN'] = cv_results.get(f'f1_{model_name}')
            sim_results[f'accuracy_{model_name}_ENN'] = cv_results.get(f'accuracy_{model_name}')
    ...
    print(f"\n{'='*25} TODAS LAS SIMULACIONES (ENN) COMPLETADAS {'='*25}")
    ...
    if all_results:
        ...
        stats_summary = "Estadísticas de Robustez (ENN + 5-Fold CV)\n"
        stats_summary += "="*60 + "\n"
        ...
        # TIEMPOS
        stats_summary += "\n### TIEMPOS ###\n"
        stats_summary += "Tiempo considerado por fold: tiempo de ENN + tiempo de entrenamiento del modelo.\n"
        ...
        if time_records:
            ...
            for _, row in resumen_tiempos.iterrows():
                stats_summary += f"--- total_time_{row['model']}_ENN ---\n"
        ...
        summary_csv_path = os.path.join(BASE_OUTPUT, 'ALL_MODELS_ENN_summary.csv')
        results_df.to_csv(summary_csv_path, index=False)
        print(f"Resultados guardados en: {summary_csv_path}")

        # Guardar resumen estadístico
        stats_summary_path = os.path.join(BASE_OUTPUT, 'Statistics_summary_ENN.txt')
