...
BASE_OUTPUT = "C:/Users/Diana/Desktop/data_balance/RUS"


# Base 2
#CSV_PATH = "C:/Users/Diana/Desktop/ieee-fraud-detection/ieee_ready_depurada.csv"
#BASE_OUTPUT = "C:/Users/Diana/Desktop/data_balance_IEEE/RUS"

# Base 3 = base 2 balanceada
#CSV_PATH = "C:/Users/Diana/Desktop/ieee-fraud-detection/ieee_balanced.csv"
#BASE_OUTPUT = "C:/Users/Diana/Desktop/data_balance_IEEE_balanced/RUS"
...
def apply_rus_on_train_data(X_train_scaled: pd.DataFrame, y_train: pd.Series, random_seed: int) -> (pd.DataFrame, pd.Series):
    ...
    logging.info(f"Tiempo de submuestreo (RUS): {resampling_time:.4f} segundos")
...
def run_cv(df, target_col, model_runners, random_seed, output_dir, time_records):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        # CAMBIO CLAVE: RUS SOLO EN TRAIN
        X_res, y_res, r_time = apply_rus_on_train_data(
        ...
        for model_name, (train_func, eval_func) in model_runners.items():
            ...
            # tiempo total por fold = tiempo de RUS + tiempo de entrenamiento del modelo.
            time_records.append({
                'tecnica': 'RUS',
...
if __name__ == "__main__":
    ...
    for i in range(1, N_SIMULATIONS + 1):
        ...
        for model_name in model_runners.keys():
            ...
            # *** Nomenclatura de columnas para RUS ***
            sim_results[f'recall_{model_name}_RUS'] = cv_results.get(f'recall_{model_name}')
            sim_results[f'roc_auc_{model_name}_RUS'] = cv_results.get(f'roc_auc_{model_name}')
            sim_results[f'pr_auc_{model_name}_RUS'] = cv_results.get(f'pr_auc_{model_name}')
            sim_results[f'precision_{model_name}_RUS'] = cv_results.get(f'precision_{model_name}')
            sim_results[f'f1_{model_name}_RUS'] = cv_results.get(f'f1_{model_name}')
            sim_results[f'accuracy_{model_name}_RUS'] = cv_results.get(f'accuracy_{model_name}')
    ...
    if all_results:
        ...
        stats_summary = "Estadísticas de Robustez (RUS + 5-Fold CV)\n"
        stats_summary += "="*60 + "\n"
        ...
        # TIEMPOS
        stats_summary += "\n### TIEMPOS ###\n"
        stats_summary += "Tiempo considerado por fold: tiempo de RUS + tiempo de entrenamiento del modelo.\n"
        ...
        if time_records:
            ...
            for _, row in resumen_tiempos.iterrows():
                stats_summary += f"--- total_time_{row['model']}_RUS ---\n"
        ...
        summary_csv_path = os.path.join(BASE_OUTPUT, 'ALL_MODELS_RUS_summary.csv')
        results_df.to_csv(summary_csv_path, index=False)
        print(f"Resultados guardados en: {summary_csv_path}")

        # Guardar resumen estadístico
        stats_summary_path = os.path.join(BASE_OUTPUT, 'Statistics_summary_RUS.txt')
